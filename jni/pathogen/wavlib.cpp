...
WAV wav_open(const char *filename, WAVError *err)
{
	//LOGI("wav_open %s 1", filename);

	WAVMode mode = WAV_READ;
    ...
    if (!filename || !strlen(filename)) {
        ...
        //fd = plat_open(filename, oflags, S_IRUSR|S_IWUSR|S_IRGRP|S_IWGRP|S_IROTH|S_IWOTH);
		/*CFile file(filename);
	...
	//LOGI("wav_open %s 2", filename);

        wav = wav_fdopen(filename, mode, err);
        if (!wav) {
            //plat_close(fd);
			//file.close();
        } else {
...
WAV wav_fdopen(const char* filename, WAVMode mode, WAVError *err)
{
	...
	wav->file.open(filename);
	
	//LOGI("wav_fdopen %s 3", filename);
	if(!wav->file.mFile)
	{
	//LOGI("wav_fdopen %s 4", filename);
        WAV_SET_ERROR(err, WAV_BAD_PARAM);
	}
	else {
		
	//LOGI("wav_fdopen %s 5", filename);
        ...
        if (mode & WAV_READ) {
	//LOGI("wav_fdopen %s 6", filename);
            if (0 != wav_parse_header(wav, err)) {
                DEL_WAV(wav);
				LOGE("wave_parse_header error %s", filename);
            } else {
...
ssize_t wav_read_data(WAV handle, uint8_t *buffer, size_t bufsize)
{
	...
	//LOGI("wav_read_data 2");
    //r = plat_read(handle->fd, buffer, bufsize);
    r = plat_read(&handle->file, buffer, bufsize);

#ifdef WAV_BIG_ENDIAN
    bswap_buffer(buffer, r);
#endif
	//LOGI("wav_read_data 3");
    return r;
}
/*
