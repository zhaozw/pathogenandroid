...
GLuint CQuake3BSP::CreateLightmapTexture(byte *pImageBits, int width, int height)
{
    GLuint texture;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 128, 128, 0, GL_RGB, GL_UNSIGNED_BYTE, pImageBits);
...
bool CQuake3BSP::LoadBSP(const char* name)
{
	...
	char bsppath[64];
	sprintf(bsppath, "maps/%s.bsp", raw);
	...
	fp.read((void*)&header, sizeof(tBSPHeader));
	fp.read((void*)&lumps, kMaxLumps*sizeof(tBSPLump));
	...
	for(i = 0; i < m_numOfVerts; i++)
	{
		...
		fp.read((void*)&m_pVerts[i], sizeof(tBSPVertex));
    ...
    fp.seek(lumps[kIndices].offset);

	//fread(m_pIndices, m_numOfIndices, sizeof(int), fp);
    fp.read((void*)m_pIndices, m_numOfIndices*sizeof(int));

	//fseek(fp, lumps[kFaces].offset, SEEK_SET);
	fp.seek(lumps[kFaces].offset);
    
	//fread(m_pFaces, m_numOfFaces, sizeof(tBSPFace), fp);
	fp.read((void*)m_pFaces, m_numOfFaces * sizeof(tBSPFace));
	...
	fp.seek(lumps[kLightmaps].offset);
    
	for(i = 0; i < m_numOfLightmaps ; i++)
	{
		//fread(&m_pLightmaps[i], 1, sizeof(tBSPLightmap), fp);
		fp.read((void*)&m_pLightmaps[i], sizeof(tBSPLightmap));
        
        m_lightmaps[i] = CreateLightmapTexture((unsigned char*)(m_pLightmaps[i].imageBits), 128, 128);
	}
	...
	fp.seek(lumps[kLeafFaces].offset);
    fp.read((void*)m_pLeafFaces, m_numOfLeafFaces * sizeof(int));
	...
	fp.seek(lumps[kPlanes].offset);
	fp.read((void*)m_pPlanes, m_numOfPlanes * sizeof(tBSPPlane));
	...
	fp.seek(lumps[kVisData].offset);
    
	if(lumps[kVisData].length)
	{
		//fread(&(m_clusters.numOfClusters),	 1, sizeof(int), fp);
		//fread(&(m_clusters.bytesPerCluster), 1, sizeof(int), fp);
		fp.read((void*)&(m_clusters.numOfClusters), sizeof(int));
		fp.read((void*)&(m_clusters.bytesPerCluster), sizeof(int));
		...
		fp.read((void*)m_clusters.pBitsets, sizeof(byte) * size);
	}
	else
	...
	fp.seek(lumps[kBrushes].offset);
	fp.read((void*)m_pBrushes, m_numOfBrushes * sizeof(tBSPBrush));
    
	m_numOfBrushSides = lumps[kBrushSides].length / sizeof(tBSPBrushSide); //sizeof(int);
	...
	fp.seek(lumps[kLightVolumes].offset);
	fp.read((void*)m_pLightVols, m_numOfLightVols * sizeof(tBSPLightVol));
...
{
    glDeleteBuffers(m_numOfFaces, m_pVertexBuffers);
    ...
    if(m_pIndexBuffers)
    {
        delete [] m_pIndexBuffers;     m_pIndexBuffers = NULL;
    }
